...
Guidance for Google Antigravity (AGY) and agents operating in this repository.
...
| Agent | Model | Role |
|---|---|---|
| `trend-scout` | Gemini 3.7 Flash (`flash`) | Scrapes peer Pages via Apify (or, for a Brand with `curated_sources` in `seeds.yaml`, digests the Operator's own curated public newsletters instead); distills the result into **Trends** |
| `idea-strategist` | Gemini 3.7 Pro (`pro`) | Turns Trends into ranked, brand-fit **Idea briefs** with a predicted **Fit Score** |
| `producer` | Gemini 3.7 Pro (`pro`) | Runs each of an accepted Idea's chosen **Recipes** (ADR-0009/0010) — two wired today: **Character Explainer with Cast** (drives its Space to a **Cast**, pauses for the Operator's **Character** pick, then renders) and the zero-gate **News Carousel** (runs straight through): authors that Recipe's **Production Spec** via its Skill, drives its Space, then composes the **Copy** outside the Space — **one Asset per chosen Recipe**. Once a Run's Assets are produced it also offers the **Schedule Batch** export, running it only after the Operator approves all of that Run's outputs and captions in-conversation (issue #148) |
| `performance-tracker` | Gemini 3.7 Flash (`flash`) | Pulls posts' **public** metrics via Apify; computes **Performance Score**; updates the feedback loop |

## The OrganicGrowth Pipeline (Weekly / Daily Loop)
...
1. **Trend Discovery & Idea Suggestion** (`.agents/skills/run-trends/SKILL.md`) → for that Brand's named **Format** (ADR-0013 — its own voice, trend
...
2. 👤 **Gate 1 — Review.** (`.agents/skills/review-ideas/SKILL.md`) → Operator accepts/rejects conversationally; every
...
3. **Production (attended, serialized).** Each accepted Idea's chosen Recipe goes on the global
   **Production Queue** (`data/queue.json`, keyed `(brand, idea, recipe)` — ADR-0009/0011). The
   ...
   Space calls as they happen. Backlog can be viewed with `.agents/skills/queue/SKILL.md`.
4. 👤 **Gate 2 — each chosen Recipe's own pick-gate(s).** For the wired *Character Explainer with Cast*
   Recipe this is the **Cast** pick (`.agents/skills/pick-cast/SKILL.md`): Operator picks the
   **Character**; the `producer` then renders the Asset in the same session — pins the Character, runs
   ...
   `.agents/skills/pick/SKILL.md` resumes ANY wired Recipe's ANY declared gate;
...
5. **Schedule Batch approval (in-conversation, producer-offered — issue #148).** Once a Run's eligible
   ...
   it is conversational only, and writes **nothing** to the ledger. Once approved, `.agents/skills/export-schedule/SKILL.md`
   hosts each eligible Asset's slides as JPGs on S3, writes two Zoho-ready CSVs (one per configured **Zoho Social Brand**) plus a manifest, and stamps `scheduled_at`
   on each exported Asset — its `status` stays `produced` (ADR-0011's lifecycle is unchanged; no new
   status). A self-cleaning cleanup pass (`.agents/skills/cleanup-schedule-media/SKILL.md`) runs automatically first,
...
6. 👤 **Gate 3 — Publish.** For a Schedule Batch Asset, the Operator uploads the exported CSVs to Zoho
   ...
   Batch approval above. Either way, once live, `.agents/skills/log-post/SKILL.md` links
...
7. **Performance Tracking** (`.agents/skills/track-performance/SKILL.md`) → `performance-tracker` pulls public metrics (Apify) for every posted
...
8. **Reporting** (`.agents/skills/report/SKILL.md`) → pipeline state, the per-Idea Fit Score vs the best measured Performance Score
...
run except when `.agents/skills/build-issue/SKILL.md` is invoked against a `ready-for-agent` GitHub issue. They are a
...
| Agent | Model | Role |
|---|---|---|
| `developer` | Gemini 3.7 Pro (`pro`) | Implements one build slice: authors an OpenSpec change from the issue, then builds it test-first against a **fake** Magnific Space |
| `qa` | Gemini 3.7 Flash (`flash`) | The only non-human gate: runs tests, verifies the acceptance criteria and that the spec matches the issue, reports a verdict; never edits code |

### Development Pipeline (Per Slice)

`.agents/skills/build-issue/SKILL.md` is the **only** trigger. One issue per run → one branch → one PR; the agents
**never self-select work**. Issue repo is `SandroBlunt/OrganicGrowth`.

1. **Pre-flight.** Refuses to run unless the issue is labeled `ready-for-agent`
...
4. **On fail.** Hands QA's defects back to the `developer`, which fixes and resubmits;
...
filled from here, read via the typed `BrandAssetStore`; ADR-0016), `baseline-prompts/<format>/<recipe>.md`
(the referenced **Baseline Prompt** document a Format's `formats/<slug>.yaml` points at per
Recipe — never inline YAML; a Recipe's producer Skill reads and interprets it to author the media
prompt, read via the typed loader `loadBaselinePrompt`; ADR-0015), `ideas/<format>/<run>/idea-NN.md` (one Brief each),
`ideas/<format>/<run>/idea-NN.<recipe>.spec.json` (a chosen Recipe's **Production Spec**, written by the `producer` when that job is produced),
`ideas/…/idea-NN.<recipe>.output/` (the Save phase's self-contained publish + tracking bundle),
`ideas/…/idea-NN.<recipe>.cast/` (a Recipe's rendered GATE-CANDIDATE images, downloaded when the ledger records the gate pause),
and `ledger.json`. The Production Queue is the one exception — it is brand-agnostic at `data/queue.json` (ADR-0006, ADR-0008).
...
`queued → in_production → produced → posted → tracking → scored`. Update the ledger on every status change.
...
- **S3** hosts a Schedule Batch export's JPGs as public direct links for Zoho Social to fetch (the Media
  Host port, issue #144). Already live for straw-motion; see [`docs/schedule-batch-s3-setup.md`](./docs/schedule-batch-s3-setup.md).

## Rules & Antigravity Customizations

Antigravity automatically discovers and loads customizations from `.agents/`:
- **Rules (`.agents/rules/`)**:
...
- **Skills (`.agents/skills/`)**:
  - Recipe craft skills (`produce-character-explainer`, `produce-news-carousel`, `produce-news-short-script`, `write-social-copy`).
  - Model prompt skills (`kling-3-0`, `veo-3-1`, `seedance-2-0`, etc.).
  - Pipeline workflow skills (`run-pipeline`, `run-trends`, `review-ideas`, `pick-cast`, `pick`, `export-schedule`, `track-performance`, `report`, `queue`, `cleanup-schedule-media`, `log-post`, `build-issue`).
- **MCP Servers (`.agents/mcp_config.json`)**: Configures `magnific` and `zoho-social` tool integrations.
- **Lifecycle Hooks (`.agents/hooks.json`)**: Enforces safety boundaries (e.g. publication gate restrictions).
