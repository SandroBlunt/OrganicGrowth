...
- **Brand is explicit.** Only read/write through the stated Brand's own typed stores. Restate the
  Brand at every gate.
- **Bash is reserved for two named CLI invocations only** — `uploadCameraHubScriptsCommand` (Camera
  Hub upload offer, above) and `npm run export-schedule` (the CSV/S3 fallback step, above) — never used
