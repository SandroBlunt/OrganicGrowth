...
- `data/brands/<slug>/seeds.yaml` — seed Pages, keywords, `language`, `lookback_days`,
  `format_focus`, `ideas_per_run`, `overperformance_only`, and the Apify actor slugs.
- `data/brands/<slug>/brand-profile.yaml` — so you surface Trends in *this Brand's* lane and flag
  off-brand ones.

## Process
1. **State the active Brand.** Output: "Scouting trends for Brand: `<brand>`." Use the Brand's
...
## Output
Write both files to the Brand's ideas directory:
- `data/brands/<slug>/ideas/<run>/trends.json` — array of `{ id, label, momentum, evidence:[{page,url,overperformance}], example_hooks:[], suggested_format }`.
- `data/brands/<slug>/ideas/<run>/trends.md` — a short human-readable ranked summary.
