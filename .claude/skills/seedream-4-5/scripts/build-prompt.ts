 ...
 *
 * Ported from build-prompt.py (issue #255) — same clause/word-budget rules, same CLI shape and exit
 ...
 * Seedream front-end (Dreamina / Jimeng, fal, wavespeed). It does NOT call any API. On-screen settings
 * (aspect ratio, resolution, image count) are recorded for the operator's reference and printed in a
 * trailing settings block; they are not part of the prompt the model reads.
 *
 * Modes: T2I, Edit, MR (reference by ordinal "Image N"), sequential-set.
 *
 * Seedream prompt rules enforced here:
 *   - subject-first natural-language prose (lead with the subject; earlier concepts are weighted more
 *     heavily).
...
const VALID_MODES = ["T2I", "Edit", "MR", "sequential-set"] as const;
type Mode = (typeof VALID_MODES)[number];

const VALID_ASPECT_RATIOS = ["1:1", "4:3", "3:4", "16:9", "9:16", "3:2", "2:3", "21:9"] as const;
...
export function buildPrompt(args: BuildPromptArgs): string {
  ...
  if (mode === "sequential-set") {
    return buildSequentialSet({ subject, style, frames, setCount, inImageText, aspectRatio, resolution });
  }

  return buildStill({ mode, subject, action, setting, style, camera, references, inImageText, aspectRatio, resolution });
}
...
function buildStill(opts: {
  ...
  const { mode, subject, action, setting, style, camera, references, inImageText, aspectRatio, resolution } = opts;
  ...
  if (inImageText && inImageText.trim()) {
    parts.push(`In-image text: ${inImageText.trim()}.`);
  }
...
function buildSequentialSet(opts: {
  ...
  const { subject, style, frames: framesArg, setCount, inImageText, aspectRatio, resolution } = opts;
...
const HELP_TEXT = `usage: build-prompt.ts --mode {T2I,Edit,MR,sequential-set} --subject SUBJECT [options]

Assemble a Seedream 4.5 prompt with validation.

options:
  --mode {T2I,Edit,MR,sequential-set}   required
  --subject SUBJECT                       required
  --action ACTION
  --setting SETTING
  --style STYLE
  --camera CAMERA
  --reference REFERENCE                     plain string or JSON {"role": "..."}; order = on-screen order.
  --frame FRAME                               per-image state for sequential-set mode; repeat per image.
...
function parseArgs(argv: readonly string[]): ParsedArgs {
  ...
  if (values["subject"] === undefined) {
    throw new PromptValidationError("the following arguments are required: --subject");
  }
