...
- **Logo:** `Straw_Motion_Logo` connected as a reference image (not described in prose), laid
  ...
  unaltered — no shape/proportion/color changes — with a soft dark gradient vignette behind it
  for legibility, never a hard-edged solid black bar or filled box.
...
> in any way, and do not restyle it to match the scene. A soft dark gradient vignette sits behind it
