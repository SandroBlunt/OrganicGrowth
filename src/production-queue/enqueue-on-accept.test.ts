...
describe("planEnqueue (pure policy: accepted-only + no-duplicate)", () => {
  it("enqueues an accepted Idea as one queued cast job", () => {
    const r = planEnqueue(IDEAS, emptyQueue(), "idea-accepted", NOW);
  ...
  it("refuses a rejected Idea — no job is produced", () => {
    const r = planEnqueue(IDEAS, emptyQueue(), "idea-rejected", NOW);
  ...
  it("refuses a still-suggested Idea", () => {
    const r = planEnqueue(IDEAS, emptyQueue(), "idea-suggested", NOW);
    assert.equal(r.enqueued, false);
    assert.equal(r.reason, "not-accepted");
    assert.equal(r.state.jobs.length, 0);
  });
  ...
  it("refuses an unknown Idea", () => {
    const r = planEnqueue(IDEAS, emptyQueue(), "idea-ghost", NOW);
    assert.equal(r.enqueued, false);
    assert.equal(r.reason, "unknown-idea");
  });

  it("does not duplicate when the accepted Idea already has a job", () => {
    const existing = enqueue(emptyQueue(), "idea-accepted", "2026-06-05T10:00:00.000Z");
    const r = planEnqueue(IDEAS, existing, "idea-accepted", NOW);
    assert.equal(r.enqueued, false);
...
describe("enqueueOnAccept (orchestration shell, real files)", () => {
  it("persists exactly one job when an accepted Idea is enqueued", async () => {
    await withTempFiles(async (ledgerPath, queuePath) => {
      const r = await enqueueOnAccept("idea-accepted", {
        ledgerPath,
        queuePath,
        now: () => NOW,
      });
  ...
  it("is idempotent on re-accept: a second call adds no job", async () => {
    await withTempFiles(async (ledgerPath, queuePath) => {
      await enqueueOnAccept("idea-accepted", { ledgerPath, queuePath, now: () => NOW });
      const second = await enqueueOnAccept("idea-accepted", {
        ledgerPath,
        queuePath,
  ...
  it("never writes a queue file for a rejected Idea", async () => {
    await withTempFiles(async (ledgerPath, queuePath) => {
      const r = await enqueueOnAccept("idea-rejected", { ledgerPath, queuePath, now: () => NOW });
      assert.equal(r.enqueued, false);
