/**
 * Tests for the typed command surface's tenancy operations (issue #204) — `createBrand`/`createFormat`/
 * `createRun`, thin shells over `src/brand/store.ts`/`src/format/store.ts`/`src/run/store.ts`.
 *
 * These three were not part of issue #205's own eight named operations (listing Trends, creating an
 ...
 * (#204) is the first caller that needs to create Brand/Format/Run rows at all, and "route every write
 * through the command surface, never directly through a store" (this ticket's own constraint) means the
 * importer cannot legally call `createBrand`/`createFormat`/`createRun` directly — so this module adds
 * the three thin shells the importer needs, following #205's own established pattern exactly.
 */
...
import { createBrand, createFormat, createRun } from "./tenancy.ts";
