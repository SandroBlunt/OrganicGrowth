...
export class LiveSpaceAdapter implements SpaceMcpPort {
  ...
  /**
   * Checks the CALLER-named `nodeName` — never hard-coded to one node regardless of caller (issue
   * #102 finding #4). Reads via `readState()`'s full-board merge; for a node name outside
   * `KEY_NODE_NAMES` (e.g. a News Carousel Recipe's "Brand_Logo") this naturally uses the plain
   * `spaces_state` inventory value — the read proven reliable for a creation-type node's bind, per
   * the live smoke test that surfaced this bug (the SCOPED `spaces_get_nodes` read was the one that
   * mis-reported it as missing).
   */
  async verifyPinned(value: string, nodeName: string): Promise<boolean> {
