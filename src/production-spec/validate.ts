 ...
 *
 * --- `ValidationResult`/`ValidationError` are the SHARED spec-validation shape (issue #81) ---
 *
 ...
 * DIFFERENT Recipe (the News Carousel, `news-carousel-validate.ts`) has its OWN error-code vocabulary
 * and reuses these two interfaces rather than inventing a parallel, redundant pair.
 */
...
/** Stable, machine-checkable identifiers for each contract violation THIS validator produces. */
