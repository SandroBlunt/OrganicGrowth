...
// Documentation-conformance suite. These assertions read markdown docs (GEMINI.md and
// .agents/skills/**/*.md) and pin their current prose. They are deliberately kept OUT of the unit
...
describe("command surface — final and matches the shipped Producer feature", () => {
  it("ships /queue, /pick-cast, /log-post and NO /produce command", async () => {
    const cmds = join(REPO_ROOT, ".agents", "skills");
    await readFile(join(cmds, "queue", "SKILL.md"), "utf8");
    await readFile(join(cmds, "pick-cast", "SKILL.md"), "utf8");
    await readFile(join(cmds, "log-post", "SKILL.md"), "utf8");
    await assert.rejects(
      readFile(join(cmds, "produce", "SKILL.md"), "utf8"),
      "there must be no /produce command file (auto-enqueue on accept replaced it)",
    );
  });

  it("GEMINI.md documents the final lifecycle and carries no stale '/produce' wiring", async () => {
    const claude = await readFile(join(REPO_ROOT, "GEMINI.md"), "utf8");
    // ADR-0011: the Idea itself only ever carries suggested/accepted/rejected...
    assert.match(
      claude,
      /suggested \/ accepted \/ rejected/,
      "GEMINI.md must document the Idea's own suggested/accepted/rejected lifecycle (ADR-0011)",
    );
    ...
    assert.match(
      ...
      "GEMINI.md must document the per-Asset production lifecycle (ADR-0011)",
    );
    assert.doesNotMatch(
      claude,
      /written by `\/produce`/,
      "GEMINI.md must not say the Production Spec is written by /produce",
    );
  });

  it("report.md describes the production states (casting/produced) and keeps Fit vs Performance distinct", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "report", "SKILL.md"), "utf8");
  ...
  it("pick-cast.md documents the <brand> argument as required", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "pick-cast", "SKILL.md"), "utf8");
  ...
  it("run-trends.md documents the <brand> argument as required", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "run-trends", "SKILL.md"), "utf8");
  ...
  it("review-ideas.md documents the <brand> argument as required", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "review-ideas", "SKILL.md"), "utf8");
  ...
  it("queue.md documents the <brand> argument as required", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "queue", "SKILL.md"), "utf8");
  ...
  it("log-post.md documents the <brand> argument as required", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "log-post", "SKILL.md"), "utf8");
  ...
  it("track-performance.md documents the <brand> argument as required", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "track-performance", "SKILL.md"), "utf8");
  ...
  it("all content agent files thread the Brand through their prompts", async () => {
    ...
    for (const agentFile of ["trend-scout", "SKILL.md", "idea-strategist", "SKILL.md", "producer", "SKILL.md", "performance-tracker", "SKILL.md"]) {
  ...
  it("producer.md restates the Brand at Gate 2 (Cast pick)", async () => {
    const doc = await readFile(join(REPO_ROOT, ".agents", "skills", "producer", "SKILL.md"), "utf8");
