...
describe("FakeMediaHost — THIS IS THE MEDIA HOST FAKE (issue #144 AC1)", () => {
  ...
  it("records every upload call and returns a public URL ending in the uploaded .jpg key", async () => {
    const host = new FakeMediaHost();
    const result = await host.upload("/tmp/slide-0.jpg", "straw-motion/2026-W32/idea-01/0-hook.jpg");

    assert.deepEqual(host.uploadCalls, [
      { localPath: "/tmp/slide-0.jpg", key: "straw-motion/2026-W32/idea-01/0-hook.jpg" },
    ]);
    assert.ok(result.url.endsWith("straw-motion/2026-W32/idea-01/0-hook.jpg"));
  ...
  it("rejects an upload key that does not end in .jpg — never hosts a link Zoho can't fetch", async () => {
    const host = new FakeMediaHost();
    await assert.rejects(() => host.upload("/tmp/slide-0.png", "not-a-jpg.png"), /\.jpg/);
  ...
  it("failUploads models an upload failure while still recording the attempted call, and does not host it", async () => {
    ...
    await assert.rejects(() => host.upload("/tmp/a.jpg", "a.jpg"));
    assert.deepEqual(host.uploadCalls, [{ localPath: "/tmp/a.jpg", key: "a.jpg" }]);
    assert.equal(host.isHosted("a.jpg"), false);
  });
