...
of the ledger's own Asset**, never a second, hand-maintained store — regenerated by
`src/asset/output-bundle.ts`'s one `generatePostJson` function at produce / `/log-post` /
`/track-performance` — git-ignored, durable on the Operator's disk; the ledger's `asset_paths` point
here), `ideas/…/idea-NN.<recipe>.cast/` (a Recipe's rendered GATE-CANDIDATE images, downloaded at the
same time the ledger records the gate pause — issue #119: a distinctly-named sibling of the `.output/`
bundle and the `.spec.json` Spec, built by `src/asset/cast-candidates.ts`'s `castCandidatesDirFor` +
`downloadCastCandidates`; each candidate's local file path is recorded on the ledger's `cast` field
alongside its existing `identifier`/`url`, and `/pick-cast` surfaces that local path once it exists),
and `ledger.json`. **Legacy layout note:** runs that predate Format-namespacing sit one level up, at
...
(ADR-0006, ADR-0008). **Likewise, an Asset produced before issue #112 keeps its OLD `.assets/`-named
directory untouched** — `refreshOutputBundle` resolves each Asset's bundle directory from its own
recorded `asset_paths` rather than reconstructing a name, so a legacy `.assets/` Asset's `post.json`
keeps refreshing right there; nothing is ever renamed or migrated on disk.
