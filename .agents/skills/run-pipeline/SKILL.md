...
name: run-pipeline
description: >-
  Start and conduct the whole weekly loop for a Brand (named, picked from a menu, or created on the fly):
  readiness check, trends, ideas, review, production, publish, and performance offers — pausing only at the three human gates.
---

# Run Pipeline Workflow

Usage: Run the pipeline for `<brand>` (or prompt without arguments to select/create a brand).
...
3. **Prints a rename hint.** Outputs a line like `Session: mundotip · 2026-W23`.
...
4. **Detects in-flight work.** Reads the ledger and queue to determine the current phase. Only genuine
   ...
   **There is no default — you must state your choice.**
...
5. **Drives the loop, pausing at three human gates:**
   ...
   - **Gate 2 — each chosen Recipe's own pick-gate(s).** Presents the Ideas with an Asset waiting at a
     gate and tells you which action to run — for the wired Recipe, `pick-cast <brand> <idea-id> <n>`
...
> gate-paused job does **not** hold the Space, so the next queued job can run.
...
- **Readiness gate is HERE only.** The granular workflow skills (`run-trends`, `review-ideas`,
