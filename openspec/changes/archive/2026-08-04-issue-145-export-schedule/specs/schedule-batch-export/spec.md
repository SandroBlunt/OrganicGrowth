...
Copy has no variant for a platform any configured Zoho Social Brand targets; an eligible Asset that does
not have exactly the News Carousel Recipe's fixed slide count of downloaded slides. It SHALL collect
EVERY problem across EVERY eligible Asset, never stopping at the first. The orchestration shell SHALL
run this BEFORE hosting any media or writing any file, and SHALL refuse the whole export (writing
nothing) when it returns any problem.
