...
#### Scenario: The workflow uses Node 22 or newer and runs npm ci then exactly npm test

- **GIVEN** `.github/workflows/ci.yml`'s `actions/setup-node` step and its `run:` steps
- **WHEN** they are read
- **THEN** `node-version` names Node 22 or newer, one step's command is `npm ci`, and a later step's
  command is exactly `npm test`
...
step's `run:` command, trimmed, is exactly `npm test`); and `referencesLiveCredentials(rawYamlText)`
(true when the raw text contains `secrets.` or a Magnific/Zoho/Apify/AWS-shaped credential name,
case-insensitively). A test in this repository's `npm test` suite SHALL read
`.github/workflows/ci.yml` from this repository's own root — resolved from the test file's own on-disk
location via `import.meta.url`, never `process.cwd()` or a hardcoded checkout path — parse it with this
module, and assert it satisfies all five checks, so a future edit that narrows the trigger, drops
`fetch-depth: 0`, changes the run command, or adds a live-credential reference is caught by this suite
rather than only discovered when CI itself goes red or, worse, silently stops being hermetic.
...
- **GIVEN** `.github/workflows/ci.yml` as shipped, read from this repository's own root resolved via
  `import.meta.url`
- **WHEN** it is parsed with `parseWorkflow` and checked with all five functions
- **THEN** `triggersOnPushAndPullRequest` is `true`, `checkoutFetchDepth` is `0`, `setupNodeVersion`
  names Node 22 or newer, `runsExactNpmTest` is `true`, and `referencesLiveCredentials` on the raw text
  is `false`
