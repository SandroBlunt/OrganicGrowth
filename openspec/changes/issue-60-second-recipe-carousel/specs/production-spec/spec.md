...
SHALL pass its VALIDATOR and the brand-safety filter; a Spec that fails either SHALL NOT be written.

`composeSpec`'s validator (like its already-injectable `generator`) SHALL be injectable via
`options.validator`, defaulting to the wired *Character Explainer with Cast* Recipe's `validate` —
UNCHANGED behavior for every existing caller that omits it (issue #60). A DIFFERENT Recipe (e.g. the
News Carousel, whose Spec shape is entirely different) SHALL supply its OWN validator — typically
`recipe.specShape.validate` (`recipe/registry.ts`) — alongside a matching `generator`; `composeSpec`
SHALL NEVER fall back to the wired Recipe's validator when a different one is injected, even if the
injected one rejects the Spec.
...
#### Scenario: A different Recipe's own injected validator gates the write, not the wired one

- **GIVEN** a candidate Spec shaped for the News Carousel Recipe (a `slides` list — NOT
  `character_concepts`/`clips`/`thumbnails`), composed with `validator: validateNewsCarouselSpec`
- **WHEN** `composeSpec` is called
- **THEN** the Spec is written when `validateNewsCarouselSpec` accepts it — even though the WIRED
  Recipe's `validate()` would reject this same Spec shape
- **AND** when the injected validator instead REJECTS the generated Spec, nothing is written and
  `composeSpec` never silently falls back to checking it against the wired Recipe's own contract
