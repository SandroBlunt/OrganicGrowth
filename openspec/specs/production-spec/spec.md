...
logoReferencePhrase, logoNameGuardrailInstruction, fixedClauses, confirmedCardStyles,
topRegionCardStyles, minDistinctCardStyles }`). It SHALL run
...
selected by array position — issue #105) with exactly 12 entries (without a supplied
`baselineDocumentText`; 13 with one), covering:
...
When `baselineDocumentText` is supplied, a 13th item, **`baseline-doc-verified`**, additionally
...
- **THEN** the result's `ok` is `true`, `items.length` is `12`, exactly one item (`id:
...
- **THEN** the result's `ok` is `true`, `items.length` is `12`, exactly one item (`id:
  "grounded-subject"`) is `kind: "agent-judged"` with `ok: null`, and every `kind: "mechanical"` item
  is `ok: true` (including `no-dash-tells`, issue #108, `logo-name-not-as-text`, issue #110, and
