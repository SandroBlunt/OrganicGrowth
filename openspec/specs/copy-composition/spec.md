...
clock read. `defaultDraftCopy` SHALL always produce a `Copy` that passes `validateCopy` (below) for the
...
proving Copy composes LATE, after the media exists. Tests SHALL exercise drafting against a deterministic
FAKE drafter standing in for the producer's LLM job — never a live model call.
...
(matched `#`-agnostically and case-insensitively); and any of `rules.bannedWords` found in the caption or
any hashtag (case-insensitive, whole-word). The banned-word check SHALL be REJECT-ONLY — it SHALL NEVER
rewrite or strip the offending text; a hit only ever fails validation.
...
- **GIVEN** a Copy whose caption is within its shape's length/emoji bounds, whose hashtags include every
  required hashtag, whose caption includes the required CTA, and which contains no banned word
